...
export default function AdminUserDetails() {
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setError(t('admin.system.invalid_response', 'User not found.'));
      }
    } catch (err) {
      console.error('Failed to fetch user details:', err);
      setError(t('admin.system.failed_stats', 'Failed to load user details.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleBan = async () => {
    ...
    const result = await Swal.fire({
      ...
      confirmButtonText: t('admin.actions.confirm_purge', 'YES_EXECUTE'),
      ...
      <HUDLayout>
        <div className="p-6 max-w-3xl mx-auto min-h-[80vh] flex items-center justify-center pt-28">
          ...
          >
            ...
            <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{error || t('admin.unknown_node')}</h2>
            <button 
        ...
        >
          ...
          >
             <div className="w-10 h-10 rounded-full bg-white/40 dark:bg-white/5 flex items-center justify-center border border-white/40 dark:border-white/10 group-hover:border-indigo-500 transition-all">
                <i className={`ph-bold ${i18n.dir() === 'rtl' ? 'ph-caret-right' : 'ph-caret-left'}`} />
             </div>
             <span className="text-[11px] font-black uppercase tracking-widest ps-1">{t('admin.back_to_registry')}</span>
          </button>
          ...
          >
             ...
             {user?.is_banned ? t('admin.restore_auth') : t('admin.restrict_account')}
          </button>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          >
            <div className="bg-white/40 dark:bg-slate-950/40 backdrop-blur-xl border border-white/40 dark:border-white/5 rounded-[32px] p-8 text-center relative overflow-hidden group shadow-premium">
              ...
              <h1 className="text-2xl font-black tracking-tighter text-slate-900 dark:text-white leading-tight">
                {user?.name || t('admin.unknown_node')}
              </h1>
              <div className="mt-4 flex flex-col items-center gap-3">
                 <span className="flex items-center gap-2 px-4 py-1.5 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-full border border-indigo-500/20 text-[10px] font-black uppercase tracking-widest">
                   <i className="ph-fill ph-briefcase" /> {user?.job_title || t('admin.sector_not_defined')}
                 </span>
            ...
            <div className="bg-white/40 dark:bg-slate-950/40 backdrop-blur-xl border border-white/40 dark:border-white/5 rounded-[32px] p-8 space-y-6 shadow-premium">
               <div className="flex items-center gap-3 mb-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t('admin.system_vitals')}</span>
               </div>
               
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/40 dark:bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                     <div className="text-[9px] font-black text-slate-400 uppercase mb-1">{t('admin.state')}</div>
                     <div className={`text-xs font-black uppercase ${user?.is_banned ? 'text-rose-500' : 'text-emerald-500'}`}>{user?.is_banned ? t('admin.banned') : t('admin.authorized')}</div>
                  </div>
                  <div className="bg-white/40 dark:bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                     <div className="text-[9px] font-black text-slate-400 uppercase mb-1">{t('admin.verified')}</div>
               ...
               <div className="space-y-4 pt-2">
                  <div className="flex items-center gap-4 text-slate-600 dark:text-slate-400">
                    ...
                    <div>
                      <div className="text-[9px] font-black uppercase tracking-tighter opacity-60">{t('admin.reg_epoch')}</div>
                  ...
                  <div className="flex items-center gap-4 text-slate-600 dark:text-slate-400">
                    ...
                    <div>
                      <div className="text-[9px] font-black uppercase tracking-tighter opacity-60">{t('admin.geo_locator')}</div>
                      <div className="text-sm font-bold text-slate-800 dark:text-slate-200">{user?.location || t('admin.node_global')}</div>
                    </div>
                  </div>
               </div>
            </div>
          ...
          >
            ...
            <div className="bg-white/40 dark:bg-slate-950/40 backdrop-blur-xl border border-white/40 dark:border-white/5 rounded-[32px] p-10 shadow-premium">
              <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-indigo-500 mb-8">{t('admin.access_protocols')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-center gap-5 p-6 bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/10 rounded-2xl group hover:border-indigo-500 transition-all">
                  ...
                  <div className="overflow-hidden">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('admin.email_mail')}</p>
                ...
                <div className="flex items-center gap-5 p-6 bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/10 rounded-2xl group hover:border-indigo-500 transition-all">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('admin.direct_link')}</p>
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{user?.phone || t('hud_labels.undefined')}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  {user?.linkedin_url && (
                    <a href={user.linkedin_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-5 p-6 bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/10 rounded-2xl hover:border-blue-500 transition-all group">
                       ...
                       <div className="overflow-hidden">
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('admin.social_net')}</p>
                  ...
                  {user?.github_url && (
                    <a href={user.github_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-5 p-6 bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/10 rounded-2xl hover:border-slate-400 transition-all group">
                       ...
                       <div className="overflow-hidden">
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('admin.repo_auth')}</p>
               ...
               <div className="flex flex-wrap gap-3">
                  {Array.isArray(user?.skills) && user.skills.length > 0 ? (
                    ...
                    <div className="w-full flex flex-col items-center justify-center py-12 text-center opacity-40">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest">{t('admin.no_extraction_data')}</p>
                    </div>
                  )}
               </div>
            </div>
