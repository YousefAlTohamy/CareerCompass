  ...
  return (
    <div className="relative w-20 h-20 flex items-center justify-center shrink-0">
      <svg className="w-full h-full -rotate-90" viewBox="0 0 72 72">
        <circle cx="36" cy="36" r="32" fill="none" stroke="currentColor" strokeWidth="6" className="text-slate-100" />
        <circle
          cx="36"
          cy="36"
          r="32"
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-lg font-black text-slate-800">{Math.round(safeScore)}%</span>
      </div>
    </div>
  );
};
...
const SkillChip = ({ skill, onRemove }) => {
  ...
  return (
    ...
    >
      ...
      <span className="font-bold text-slate-700 text-[11px] uppercase tracking-wider">{name}</span>
      {onRemove && skill?.id && (
        <button
...
const WelcomeBanner = ({ onUpload, uploading }) => (
  <motion.div
    ...
    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
      <div className="flex-1 text-center md:text-left space-y-4">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/15 text-indigo-100 text-xs font-bold uppercase tracking-wider">
          <Sparkles size={14} /> Unlock Your Potential
        </div>
        <h2 className="text-2xl md:text-4xl font-black text-white leading-tight">
          Upload your CV to unlock AI-powered insights, gap analysis, and tailored job matches.
        </h2>
        <p className="text-indigo-100/90 text-sm md:text-base font-medium max-w-xl">
          Get personalized recommendations, skill extraction, and market-ready analysis in seconds.
        </p>
        <div className="flex flex-wrap gap-3 pt-2">
          ...
          >
            <FileText size={18} /> Go to Profile
          </Link>
          <label className="cursor-pointer inline-block">
            ...
            <span className="inline-flex items-center gap-2 bg-indigo-500/80 hover:bg-indigo-500 text-white font-bold py-3.5 px-6 rounded-xl transition-colors border border-white/20">
              <Upload size={18} /> {uploading ? "Uploading..." : "Upload CV Now"}
            </span>
          </label>
        </div>
      </div>
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto pb-20 space-y-8 font-sans bg-slate-50 min-h-screen">
      ...
      <div className="space-y-6">
        {/* WELCOME BANNER (when no cv_analysis) */}
        {!hasCvAnalysis && (
          <WelcomeBanner onUpload={handleCVUpload} uploading={uploading} />
        )}
        ...
        {/* AI INSIGHTS SNAPSHOT (when we have headline, seniority, or top skills) */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
            className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4 group"
          >
          ...
          >
            ...
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Experience</h3>
            <p className="text-xl font-black text-slate-800 mt-0.5">
              {formatExperienceYears(totalExperience) ?? <span className="text-slate-300 italic">—</span>}
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group"
          >
            ...
            <div>
              ...
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Top Skills</h3>
              <p className="text-xl font-black text-slate-800 mt-0.5">
                {loading ? "…" : `${skillsCount} Skills`}
              </p>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow group"
          >
            <div className="w-12 h-12 rounded-2xl bg-fuchsia-50 text-fuchsia-600 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              <Target size={24} />
            </div>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Target Role</h3>
            <p className="text-xl font-black text-slate-800 mt-0.5 line-clamp-1" title={headline ?? "Unset"}>
              {headline ?? <span className="text-slate-300 italic">Not set</span>}
            </p>
          </motion.div>
        </div>
          ...
          >
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200">
              <h3 className="text-xl font-black text-slate-800 mb-6 flex items-center justify-between">
                ...
                <span className="text-xs font-bold bg-slate-100 text-slate-500 px-3 py-1 rounded-md">
                  {(displaySkills?.length ?? 0)} tracked
                </span>
              </h3>
          ...
          <aside className="lg:col-span-4 space-y-6">
                ...
                {(recommendations?.length ?? 0) > 0 ? (
                  ...
                  <div className="py-8 text-center text-slate-400 text-sm">
                    {loading
                      ? "Analyzing gaps…"
                      : hasSkills
                        ? "All caught up! Excellent profile match."
                        : "Upload CV to view personalized gaps."}
                  </div>
                )}
              </div>
            </motion.div>
            ...
            >
              ...
              <nav className="space-y-2">
                <Link
                  to="/jobs"
                  className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/50 transition-all group"
                >
